...
<body>
	...
	1. 1부터 100까지의 짝수 합: ${sum}
	<br>
	<c:set var="sum" value="${0 }"></c:set>
	<c:forEach var="i" begin="2" end="100" step="2">
		<c:set var="sum" value="${sum+i}"></c:set>
	</c:forEach>
	2. 1부터 100까지의 짝수 합: ${sum}
	<br>
	...
	1. 1부터 100까지의 홀수 합: ${sum}
	<br>
	<c:set var="sum" value="${0 }"></c:set>
	<c:forEach var="i" begin="1" end="100" step="2">
		<c:set var="sum" value="${sum+i}"></c:set>
	</c:forEach>
	2. 1부터 100까지의 홀수 합: ${sum}
	<br />	<%-- empty tag : 하위태그가 없는 태그 --%>
	<hr>
	<%
	...
	<h3>forEach 태그를 이용하여 list 객체를 한 줄에 2개씩 출력하기</h3>
	<c:forEach var="i" items="${list }" varStatus="stat">
		${i} &nbsp;&nbsp;&nbsp;
