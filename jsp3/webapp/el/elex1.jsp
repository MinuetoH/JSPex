...
tel 변수값 : <%=tel %><br>
없는 속성 값 : <%=pageContext.getAttribute("noAttr") %><br>
...
today 속성 값 : ${sessionScope.today}<br>
today 속성 값 : ${today}<br>
...
test 속성값 : ${test}<br>
없는 속성 값 : ${pageScope.noAttrr}<br>
없는 파라미터 값 : ${param.noParam}<br>
<%-- 
	${tel} : 영역담당 갹체에 저장된 속성 중 이름이 tel인 속성의 값을 출력
	${test} : 영역담당 갹체에 저장된 속성 중 이름이 test인 속성의 값을 출력
	- 1. pageContext에 등록된 속성 중 test인 속성의 값 출력
	...
	영역 담당 객체의 속성 정보를 저장하는 EL의 기본 객체
		pageScope : pageContext 객체의 등록된 속성 목록. ${pageScope.속성명}
		requestScope : request 객체의 등록된 속성 목록. ${requestScope.속성명}
		sessionScope : session 객체의 등록된 속성 목록. ${sessionScope.속성명}
		applicationScope : application 객체의 등록된 속성 목록. 
