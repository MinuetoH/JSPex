...
public class BoardController extends MskimRequestMapping{
	...
	public String writeForm(HttpServletRequest request,
		...
		/*
		 * boardid=1인 경우 관리자가 아니면 등록 못하도록 수정
		 */
		if(boardid.equals("1")) {
	...
	@RequestMapping("list")
   	public String list(HttpServletRequest request, HttpServletResponse response) {
		...
		if (boardid == null) boardid = "1"; 
		int pageInt = 1;	//현재페이지
		int limit = 10; 	//한 페이지당 출력되는 게시물의 건수
		//pageNum 파라미터 존재
 ...
 */
		...
		request.setAttribute("pageInt", pageInt); 	  //현재 페이지
		...
		request.setAttribute("bottomLine", bottomLine);//한 페이지에 보여질 페이지 갯수
	 ...
	 * 1. 파일 업로드 하기
       ...
       4. 비밀번호 일치
          ...
          boolean BoarDao.update(Board)
          	수정성공: 수정성공 메세지 출력 후 info 페이지 이동
